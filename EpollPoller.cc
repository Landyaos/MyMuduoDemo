...
void EpollPoller::Poll(std::vector<Channel *> &active_channels)
{
    ...
    if (num_events > 0)
    {
        ...
        for (int i = 0; i < num_events; ++i)
        {
            ...
            std::cout << epollfd_ << "event happened on sockfd " << channel_ptr->fd() << std::endl;
        }
...
void EpollPoller::Update(int operation, std::shared_ptr<Channel> channel_ptr)
{
    ...
    if (ret < 0)
    {
        ...
        std::cout << "epoll update epollfd " << epollfd_ << " operation  " << operation << " channel sockfd  " << channel_ptr->fd() << std::endl;
}
