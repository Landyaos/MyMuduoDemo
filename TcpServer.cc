...
TcpServer::~TcpServer()
{
    for (auto& item : connection_map_)
    {
        ...
        item.second.reset();
        conn->get_event_loop_ptr_()->RunInLoop(
...
void TcpServer::HandleConnection(int sockfd, const sockaddr_in client_addr)
{
    ...
    std::cout << "new connnection : " << name << " bind " << std::endl;
    ...
    conn->set_connection_cb_(connection_cb_);
    conn->set_message_cb_(message_cb_);
    conn->set_write_complete_cb_(write_complete_cb_);
    conn->set_close_cb_(
...
void TcpServer::RemoveConnectionInLoop(const std::shared_ptr<TcpConnection> conn)
{
    ...
    conn->get_event_loop_ptr_()->QueueInLoop(std::bind(&TcpConnection::ConnectionDestroyed, conn));
}
